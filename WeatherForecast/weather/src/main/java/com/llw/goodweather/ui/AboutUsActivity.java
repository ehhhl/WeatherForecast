...
public class AboutUsActivity extends BaseVBActivity<ActivityAboutUsBinding> implements View.OnClickListener {
    ...
    public void initData() {
        ...
        //蓝色状态栏
        StatusBarUtil.setStatusBarColor(context,com.llw.mvplibrary. R.color.about_bg_color);
        //设置文字下划线
    ...
    public void onClick(View v) {
        int id = v.getId();
        if (id == R.id.lay_app_version) {//版本更新
            if (is_update) {
                showUpdateAppDialog(updateUrl, updateLog);
            } else {
                ToastUtils.showShortToast(context, "当前已是最新版本");
            }
        } else if (id == R.id.tv_blog) {//博客地址
            jumpUrl(CSDN_BLOG_URL);
        } else if (id == R.id.tv_code) {//源码地址
            jumpUrl(GITHUB_URL);
        } else if (id == R.id.tv_copy_email) {//复制邮箱
